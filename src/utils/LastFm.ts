...
import type { Options } from '@/components/ScrobbleOptions.astro'
import cryptoJS from 'crypto-js'
const MD5 = cryptoJS.MD5 // note: must be imported this way to work with Vite
...
export const authLastFM = async () => {
	...
	if (!token) {
		...
		window.location.href = `https://www.last.fm/api/auth?api_key=${import.meta.env.PUBLIC_LASTFM_API}&cb=${location}`
	}
}
...
export const checkForAuthToken = async () => {
	...
	if (token) {
		...
		const query: { [key: string]: string | number } = {
			...
			api_key: import.meta.env.PUBLIC_LASTFM_API as string,
...
export const createLastFMSignature = (body: { [key: string]: string | number }) => {
	...
	const signature = sortedBody.join('') + import.meta.env.PUBLIC_LASTFM_SECRET
...
export const scrobbleAlbum = async (
	...
	unwantedTracks: if (hasOptions) {
		...
		if (options?.sides && Object.values(options.sides).length === sideTotal) {
	...
	// Note: handled after tracks are filtered
	let albumDuration = 0
	...
	tracks.forEach((track, index) => {
		...
		query[`album[${index}]`] = album
		query[`artist[${index}]`] = artist
	...
	query.api_key = import.meta.env.PUBLIC_LASTFM_API as string
